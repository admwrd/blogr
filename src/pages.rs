...
pub fn hbs_admin_page(conn: DbConn, admin: AdminCookie, user: Option<UserCookie>) -> Template {
    ...
    let output: Template = hbs_template(TemplateBody::General(format!("Welcome Administrator {user}.  You are viewing the administrator dashboard page.", user=username)), Some("Administrator Dashboard".to_string()), Some(admin), user, None, Some(start));
...
pub fn hbs_admin_retry(conn: DbConn, user: Option<UserCookie>, fail: AuthFailure) -> Template {
    ...
    let clean_msg = if fail.msg != "" { Some(input_sanitize(fail.msg)) } else { None };
...
pub fn hbs_user_page(conn: DbConn, admin: Option<AdminCookie>, user: UserCookie) -> Template {
    ...
    let output: Template = hbs_template(TemplateBody::General(format!("Welcome {user}.  You are viewing your dashboard page.", user=username)), Some("User Dashboard".to_string()), admin, Some(user), None, Some(start));
...
pub fn hbs_articles_page(page: ViewPage, conn: DbConn, admin: Option<AdminCookie>, user: Option<UserCookie>) -> Template {
    ...
    // Todo: Change title to: Viewing Article Page x/z
    let output: Template = hbs_template(TemplateBody::General("You are viewing paginated articles.".to_string()), Some("Viewing Articles".to_string()), admin, user, None, Some(start));
...
pub fn hbs_articles_tag(tag: Tag, conn: DbConn, admin: Option<AdminCookie>, user: Option<UserCookie>) -> Template {
    ...
    if results.len() != 0 {
        ...
        output = hbs_template(TemplateBody::General(alert_danger("Could not find any articles with the specified tag.")), Some(format!("Could not find any articles with the tag(s): {}", medium_sanitize(tag.tag) )), admin, user, None, Some(start));
    }
...
pub fn hbs_about(conn: DbConn, admin: Option<AdminCookie>, user: Option<UserCookie>) -> Template {
    // don't forget to put the start Instant in the hbs_template() function
    hbs_template(TemplateBody::General("This page is not implemented yet.  Soon it will tell a little about me.".to_string()), Some("About Me".to_string()), admin, user, None, None)
}
...
pub fn hbs_index(conn: DbConn, admin: Option<AdminCookie>, user: Option<UserCookie>, flash: Option<FlashMessage>) -> Template {
    ...
    if results.len() != 0 {
        ...
        output = hbs_template(TemplateBody::General("There are no articles.<br>\n<a href =\"/insert\">Create Article</a>".to_string()), None, admin, user, None, Some(start));
    } else {
        output = hbs_template(TemplateBody::General("There are no articles.".to_string()), None, admin, user, None, Some(start));
    }
